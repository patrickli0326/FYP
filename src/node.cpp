...
//    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>(lidar_scan, 1000);
    ros::Publisher scan_pub = nh.advertise<sensor_msgs::LaserScan>("scan_lidar", 1000);
   ...
   //接收解析线程
	...
	TLidarError retvalue;
  	...
  	//扫描模式命令帧：AA 08 00 04 01 01 00 02 B9 00  
	...
	//设置停止扫描 命令帧 AA 08 00 04 01 01 00 00 B8 00   返回帧 AA 08 00 04 41 01 00 00 F8 00 
	#if 0
	...
	#if 1
	int speed = 8;
	retvalue = robotics_lidar.setLidarRotationlSpeed(speed);
	if(retvalue == EXECUTE_SUCCESS )
		printf("RotationlSpeed set successs!\n");
	else
		printf("RotationlSpeed set fail! ...TLidarError=%d...target=%d\n",retvalue, speed);
	#endif
